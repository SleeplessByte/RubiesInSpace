require_relative '../env/space.universe'
...
module Generators
	...
	module Universe
		...
		class Basic
			...
			## 
			# Returns the size for the universe
			#
			def self.size( universe )
				...
				results[ :f ] = ( results[ :f ] + Space::Universe.rand( face_variance ) - face_variance / 2.to_f ).round
			...
			##
			# Returns the node types for the universe
			#
			def self.contents( universe, universe_size )
				...
				results[ :stars ] = ( results[ :stars ] + Space::Universe.rand( star_variance ) - star_variance  / 2.to_f ).round
				results[ :planets ] = ( results[ :planets ]  + Space::Universe.rand( planet_variance ) - planet_variance / 2.to_f ).round
			...
			def self.create_star( nodes, universe, universe_size, universe_contents )
				...
				result = Space::Star.new( Space::Universe.bytes Space::Universe::ENTROPY )
			...
			def self.create_planet( nodes, universe, universe_size, universe_contents )
				...
				result = Space::Planet.new( Space::Universe.bytes Space::Universe::ENTROPY )
			...
			def self.create_asteroid( nodes, universe, universe_size, universe_contents )
				...
				result = Space::Asteroid.new( Space::Universe.bytes Space::Universe::ENTROPY )
			...
			def self.create_path( left, right, universe, universe_size )
				...
				result = Space::Path.new left, right, Space::Universe.rand( universe[ :distances ][ :max ] - universe[ :distances ][ :min ] ) + universe[ :distances ][ :min ]
				left.connect result
			...
			def self.get_cosy_stars( nodes, universe )
				...
				results = nodes.select { |star| Space::Universe.rand( universe[ :dice ][ :lonely_star ] ) > 0 }
			...
			def self.populate_cosy_stars( nodes, cosy_stars, universe, universe_size, universe_contents )
				...
				universe_contents[ :planets ].times do 
					star = cosy_stars[ Space::Universe.rand cosy_stars.length ]
					planet = create_planet( nodes, universe, universe_size, universe_contents )
					path = create_path( planet, star, universe, universe_size )
				end
			...
			def self.give_clusters_an_asteroid( nodes, clusters, universe, universe_size, universe_contents )
				...
				clusters.each do | c |
					...
					cluster_object = c[ Space::Universe.rand c.length ]
			...
			def self.assign_sparse_asteroids( universe, universe_contents )
				...
				universe_contents[ :sparse_asteroids ] = ( universe_contents[ :sparse_asteroids ] + Space::Universe.rand( sparse_variance ) - sparse_variance  / 2.to_f ).round
			...
			##
			# Connect asteroids to unconnected clusters
			#
				...
				while universe_contents[ :asteroids ] > 0
					asteroids.each do | meta_a |
						...
						if universe_contents[ :asteroids ] > 0 
							...
							if Space::Universe.rand( 100 ) < ( clusters.length / universe_contents[ :asteroids ] * 100 )
			...
			##
			# Connect two clusters using a asteroid
			#
			def self.connect_clusters( asteroid, asteroid_cluster, clusters, universe, universe_size, universe_contents )
				cluster = clusters[ Space::Universe.rand clusters.length ]
				connectee = cluster[ Space::Universe.rand cluster.length ]
			...
			def self.ensure_connected_clusters( clusters, universe, universe_size )
				...
				while clusters.length > 1
					...
					left_cluster = clusters[ Space::Universe.rand clusters.length ]
					options = clusters.select { | c | c != left_cluster }
					right_cluster = options[ Space::Universe.rand options.length ]
					
					# Find the nodes to connect
					left_nodes = left_cluster.select { | n | !n.is_a? Space::Star }
					right_nodes = right_cluster.select { | n | !n.is_a? Space::Star }
					left_node = left_nodes[ Space::Universe.rand left_nodes.length ]
					right_node = right_nodes[ Space::Universe.rand right_nodes.length ]
			...
			def self.connect_sparse_asteroids( nodes, clusters, universe, universe_size, universe_contents )
				...
				while universe_contents[ :asteroids ] > 0
					...
					cluster_object = cluster[ Space::Universe.rand cluster.length ]
			...
			def self.overconnect_close_nodes( clusters, universe, universe_size )
				...
				overless_connected = clusters.first.select { | n | n.is_a? Space::Asteroid }
				while universe_size[ :p ] > 0 and overless_connected.length > 0
					...
					# Get the left object
					left_object = overless_connected[ Space::Universe.rand overless_connected.length ]
					...
					# Right object
					right_object = options[ Space::Universe.rand options.length ]
			...
			def self.build( options )
				...
				Space::Universe.log "Creation of the Universe has started with #{ options }"

				##
				#
				#
				results = Space::Universe.new( options )
				universe = results.configuration
				universe_size = size( universe )
				universe_contents = contents( universe, universe_size )

				Space::Universe.log "Developing #{  universe_contents[ :stars ] } stars, mending #{  universe_contents[ :planets ] } planets, throwing #{  universe_contents[ :asteroids ] } asteroids, " +
				...
				print "\r                                                            \r"
				Space::Universe.log "The Universe has been created"
				results.fill nodes
			...
			def self.recursive_traverse( travelling, node, endpoints, visited )
				...
				visited.push node
				endpoints.push node if node.is_a? Space::Asteroid
				node.paths.each { |p| 
